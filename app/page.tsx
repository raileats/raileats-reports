...
// Local Main Report worksheet builder for the combined All Reports workbook.
// Kept inside page.tsx so the existing mainReportGenerator.ts does not need to change.
const buildMainReportDateWiseSheetLocal = (() => {
  ...
  const buildMainReportDateWiseSheetLocal = (masterData: any[], feedbackData: any[] = []) => {
    ...
    // Same column widths as the standalone Main Report workbook.
    ws['!cols'] = [
      { wch: 18 },
      { wch: 8 }, { wch: 8 }, { wch: 8 }, { wch: 8 }, { wch: 9 },
      { wch: 8 }, { wch: 8 }, { wch: 8 }, { wch: 8 }, { wch: 8 },
      { wch: 10 }, { wch: 11 }, { wch: 8 },
      { wch: 10 }, { wch: 11 }, { wch: 8 }, { wch: 9 },
      { wch: 9 }, { wch: 9 }, { wch: 8 }, { wch: 9 },
      { wch: 9 }, { wch: 10 }, { wch: 8 }, { wch: 9 },
  ...
  return buildMainReportDateWiseSheetLocal;
})();
...
export default function Page() {
  ...
  const stationReportColumns = useMemo(() => [
    'Station Code','Rank','Station Name','Station Rank','Vendor Price','Final Base Price',
    'Final Total Commission','Final IRCTC Comm','Final RF Commission','Final GST','Final Discount',
    'Final Vendor Discount','Final RF Discount','Delivery Charges','Final Selling Price','Final Order Total',
    'Discounted Base Price','PPD','COD','Meals','Check','Count of Delivered Orders','Not Delivered Order',
    'Not Delivered %','PPD % of Final Selling Price','Feedback Good','Feedback Bad','Count of Delivered Outlets','Total Station Vendors'
  ], []);

  const stationFilterOptions = useMemo(() => {
    const result: Record<string, { key: string; label: string }[]> = {};
    stationReportColumns.forEach((column) => {
      const seen = new Map<string, string>();
      stationSummary.forEach((row: any) => {
        const raw = row[column];
        const key = raw === null || raw === undefined || String(raw).trim() === '' ? '__BLANK__' : String(raw);
        if (!seen.has(key)) {
          seen.set(key, key === '__BLANK__' ? '(Blanks)' : (typeof raw === 'number' ? raw.toLocaleString('en-IN', { maximumFractionDigits: 2 }) : key));
        }
      });
      result[column] = Array.from(seen.entries()).map(([key, label]) => ({ key, label }))
        .sort((a, b) => {
          if (a.key === '__BLANK__') return 1;
          if (b.key === '__BLANK__') return -1;
          return a.label.localeCompare(b.label, undefined, { numeric: true, sensitivity: 'base' });
    ...
    return result;
  }, [stationSummary, stationReportColumns]);

  const filteredStationSummary = useMemo(() => {
    const q = searchTerm.trim().toLowerCase();
    return stationSummary.filter((row: any) => {
      const matchesSearch = !q || stationReportColumns.some((column) =>
        String(row[column] ?? '').toLowerCase().includes(q)
      );
      if (!matchesSearch) return false;
      return stationReportColumns.every((column) => {
  ...
  }, [stationSummary, searchTerm, stationColumnFilters, stationReportColumns]);

  const stationReportTotals = useMemo(() => {
    const totals: Record<string, any> = {};
    stationReportColumns.forEach((column) => { totals[column] = ''; });
    totals['Station Code'] = 'TOTAL';
    totals['Station Name'] = `${stationSummary.length} Stations`;
    const nonNumeric = new Set(['Station Code','Station Name','Check','Not Delivered %','PPD % of Final Selling Price']);
    stationReportColumns.forEach((column) => {
      if (!nonNumeric.has(column)) {
        totals[column] = stationSummary.reduce((sum: number, row: any) => sum + (Number(String(row[column] ?? '').replace(/,/g, '')) || 0), 0);
      }
    });
    totals['Rank'] = '';
    totals['Station Rank'] = '';
    totals['Check'] = '';
    totals['Not Delivered %'] = '';
    totals['PPD % of Final Selling Price'] = '';
    return totals;
  }, [stationSummary, stationReportColumns]);
  ...
  const toggleStationFilterValue = (column: string, key: string) => {
    setStationColumnFilters((previous) => {
      ...
      if (current === undefined) return { ...previous, [column]: allKeys.filter((value) => value !== key) };
      const next = current.includes(key) ? current.filter((value) => value !== key) : [...current, key];
      if (next.length === allKeys.length) { const copy = { ...previous }; delete copy[column]; return copy; }
      return { ...previous, [column]: next };
    });
  };
              ...
              >
                ...
                {(selectedReport === 'STATION_REPORT' || selectedReport === 'LAST_DAY_STATION') && (() => {
                  ...
                  if (isLastDay) {
                    ...
                    const rows = lastDayStationSummary.filter((s: any) => String(s['Station Code'] || '').toLowerCase().includes(searchTerm.toLowerCase()));
                    return (
                      <table className="portal-report-table portal-table-station w-full min-w-[3000px] text-left border-separate border-spacing-0 text-xs whitespace-nowrap">
                        <thead className="sticky top-0 z-10 text-slate-700"><tr>{columns.map((col) => <th key={col} className="p-3 font-semibold text-center">{col}</th>)}</tr></thead>
                        <tbody>{rows.map((row: any, i: number) => <tr key={`${row['Station Code']}-${i}`} className="portal-data-row">{columns.map((column: string) => <td key={column} className="p-3">{typeof row[column] === 'number' ? row[column].toLocaleString('en-IN', { maximumFractionDigits: 2 }) : (row[column] ?? '-')}</td>)}</tr>)}</tbody>
                      </table>
                    );
                  }
                      ...
                      <table className="portal-report-table portal-table-station w-full min-w-[3000px] text-left border-separate border-spacing-0 text-xs whitespace-nowrap">
                        <thead className="sticky top-0 z-20 text-slate-700">
                          <tr className="bg-slate-300 font-extrabold text-slate-950">
                            {stationReportColumns.map((column) => <th key={`total-${column}`} className="p-2 border-b border-slate-400 text-center">{column === 'Station Code' ? 'TOTAL' : column === 'Station Name' ? `${stationSummary.length} Stations` : (stationReportTotals[column] ?? '')}</th>)}
                          </tr>
                          <tr className="bg-slate-100">
                            {stationReportColumns.map((column) => {
                              const isFiltered = stationColumnFilters[column] !== undefined;
                              const selectedCount = stationColumnFilters[column]?.length ?? (stationFilterOptions[column]?.length ?? 0);
                              return (
                                <th key={column} className="p-2 font-semibold text-center relative border-b border-slate-300">
                                  <div className="flex items-center justify-center gap-1">
                                    <span>{column}</span>
                                    <button type="button" title={`Filter ${column}`} aria-label={`Filter ${column}`} onClick={(event) => openStationColumnFilter(column, event)} className={`inline-flex h-6 w-6 shrink-0 items-center justify-center rounded ${isFiltered ? 'bg-amber-400 text-slate-900' : 'bg-white text-slate-500 border border-slate-300'} hover:bg-amber-100`}>▼</button>
                                  </div>
                                  {isFiltered && <div className="mt-0.5 text-[9px] text-amber-600">{selectedCount} selected</div>}
                                </th>
                              );
                            })}
                          </tr>
                        ...
                        <tbody>
                          {filteredStationSummary.map((row: any, i: number) => {
                            const base = Number(row['Final Base Price'] || 0);
                            const businessRank = Number(row['Rank']);
                            const stationRank = Number(row['Station Rank']);
                            const rowColor = base === 0 ? '#dc2626' : (Number.isFinite(stationRank) && businessRank <= stationRank ? '#166534' : '#78350f');
                            return (
                              <tr key={`${row['Station Code']}-${i}`} className="portal-data-row">
                                {stationReportColumns.map((column: string, j: number) => {
                                  const value = row[column];
                                  return <td key={column} style={{ color: rowColor }} className={`p-2 ${j < 4 ? 'font-semibold' : ''} ${typeof value === 'number' ? 'text-right' : ''}`}>{typeof value === 'number' ? value.toLocaleString('en-IN', { maximumFractionDigits: 2 }) : (value ?? '-')}</td>;
                                })}
                              </tr>
                            );
                          })}
