...
export interface VendorReportRow {
  ...
  'Previouse Balance': number;
  'Paid to Vendors By Relfood': number;
  'Payment Received from Vendor to Relfood': number;
  'Net Payment': number;
}
...
export const generateVendorWiseData = (
  ...
  Object.entries(outletOrdersMap).forEach(([outletId, orders]) => {
    ...
    // Vendor RDS payment fields are already stored in Current Month Data by Outlet ID.
    // Carry the same values into Vendor Report without recalculating or changing them.
    const previousBalance = Number(currentMonthMap[outletId]?.previousBalance ?? currentMonthMap[outletId]?.['Previouse Balance'] ?? currentMonthMap[outletId]?.['Previous Balance'] ?? 0) || 0;
    const paidToVendors = Number(currentMonthMap[outletId]?.paidToVendors ?? currentMonthMap[outletId]?.['Paid to Vendors By Relfood'] ?? 0) || 0;
    const paymentReceivedFromVendor = Number(currentMonthMap[outletId]?.receivedFromVendor ?? currentMonthMap[outletId]?.['Payment Received from Vendor to Relfood'] ?? 0) || 0;
    ...
    rawOutletList.push({
      ...
      paidToVendors: Number(paidToVendors.toFixed(2)),
      paymentReceivedFromVendor: Number(paymentReceivedFromVendor.toFixed(2)),
    });
  });
  ...
  return rawOutletList.map((row) => ({
    ...
    'Previouse Balance': row.previousBalance,
    'Paid to Vendors By Relfood': row.paidToVendors,
    'Payment Received from Vendor to Relfood': row.paymentReceivedFromVendor,
